...
impl BlendRouterMock {
    ...
    fn read_reserve(env: &Env, asset: &Address) -> Reserve {
        env.storage().instance().get(&DataKey::Reserve(asset.clone())).unwrap_or(Reserve {
            asset: asset.clone(),
    ...
    pub fn set_reserve(
        ...
        let mut reserve_assets: Vec<Address> = env.storage().instance().get(&DataKey::ReserveAssets).unwrap_or(Vec::new(&env));
        let mut found = false;
        ...
        if !found {
            ...
            env.storage().instance().set(&DataKey::ReserveAssets, &reserve_assets);
        }
        env.storage().instance().set(&DataKey::Reserve(asset.clone()), &Reserve {
            asset,
            config: ReserveConfig {
                c_factor,
                decimals: 7,
    ...
    pub fn submit(env: Env, from: Address, spender: Address, to: Address, requests: Vec<Request>) -> i128 {
        ...
        for req in requests.iter() {
            ...
            match req.request_type {
                2 => {
                    ...
                    let _ = env.invoke_contract::<()>(&req.address, &symbol_short!("transfer"), args);
        ...
        env.events().publish((symbol_short!("blend"),), (from, to, last_out));
        last_out
    }
    ...
    pub fn execute_action(
        ...
        env.events()
            .publish((symbol_short!("blend"),), (caller, action, market_id, amount, receiver, out));
        out
    }
    ...
    pub fn get_positions(env: Env, address: Address) -> Positions {
        ...
        let reserve_assets: Vec<Address> = env.storage().instance().get(&DataKey::ReserveAssets).unwrap_or(Vec::new(&env));
