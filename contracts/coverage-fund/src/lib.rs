...
mod test {
    ...
    fn test_bootstrap_reward_flow_remains_compatible() {
        let (_env, fund_id, fund, reserve_token, bootstrap_token, admin, staker, _treasury) =
            setup();
    ...
    fn test_premium_routing_splits_between_reserve_rewards_and_treasury() {
        let (_env, fund_id, fund, reserve_token, _bootstrap_token, admin, staker, treasury) =
            setup();
    ...
    fn test_premium_distribution_waits_for_first_staker() {
        let (_env, fund_id, fund, reserve_token, _bootstrap_token, admin, staker, _treasury) =
            setup();
        let vault = Address::generate(&fund.env);
        let payer = Address::generate(&fund.env);
