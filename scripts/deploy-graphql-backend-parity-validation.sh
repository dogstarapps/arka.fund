...
OUT_JSON="${OUT_JSON:-$ROOT_DIR/tmp/graphql-backend-parity.json}"
...
echo "2) Validate native vs GraphQL backend parity against canonical testnet registry"
node "$SERVICE_DIR/scripts/graphql-parity-validation.mjs" \
  --deploy-json "$DEPLOY_JSON" \
  --out-json "$OUT_JSON" \
  --rpc-url "$RPC_URL" \
  --network-passphrase "$NETWORK_PASSPHRASE" \
  --graphql-profile "generic" \
  --mirror-profile "generic" \
  --validation-key "graphqlBackendParity" \
  --update-deployments

echo "GraphQL backend parity validation written to $OUT_JSON"
