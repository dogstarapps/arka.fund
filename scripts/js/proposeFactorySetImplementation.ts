...
async function main() {
  ...
  const implHashHex = getRequiredEnv('IMPL_HASH_HEX').replace(/^0x/i, '')
  ...
  const assembled = await client.propose({
    creator,
    title: 'Set Arka Implementation',
    description: `Governed factory implementation update for ${factoryId}`,
    action,
